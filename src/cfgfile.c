...
static void _set_defaults(ice_config_t *configuration)
{
    ...
    configuration->listeners_handler = NULL;
        ...
        ice_config_t *configuration)
{
    do {
        ...
        } else if (xmlStrcmp (node->name, XMLSTR("listeners-handler")) == 0) {
            if (configuration->listeners_handler) xmlFree (configuration->listeners_handler);
            configuration->listeners_handler = (char *)xmlNodeListGetString(doc, node->xmlChildrenNode, 1);
